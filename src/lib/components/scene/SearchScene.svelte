<script lang="ts">
	import { fetchArtists } from '../../../API/APIManager';
	import type { APIArtist } from '../../../api_models/APIArtist';
    ...
    */
	...
	async function search() {
		...
		switch (query.kind) {
			case QUERY_KIND.ARTIST:
                const result = await fetchArtists({filters: {
                    text_query: query.searchQuery,
                }, page: 1, limit: 50});
    ...
    function displayArtists(artists: APIArtist[]) {
        console.log(artists);
    }
</script>
